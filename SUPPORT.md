# Support

## Documentation

- [README](README.md) — overview and quick start
- [docs/README.md](docs/README.md) — operator guides
- [docs/FEATURES.md](docs/FEATURES.md) — feature checklist
- [docs/VPS-SMOKE-TEST.md](docs/VPS-SMOKE-TEST.md) — automated VPS testing
- [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md) — design overview

## Community

- **Bug reports / features:** GitHub Issues
- **Questions / discussion:** GitHub Discussions (when enabled)
- **Pull requests:** see [CONTRIBUTING.md](CONTRIBUTING.md)

## Security

Sensitive reports: [SECURITY.md](SECURITY.md)

## Commercial / consulting

Dockfin is free and open source. Maintainers may offer paid consulting separately; there is no paid product tier inside Dockfin itself.
