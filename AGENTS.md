...
Use when serving the built SPA from the API (`DOCKFIN_WEB_DIR`), e.g. VPS smoke at `/opt/dockfin-smoke`.

For the **Docker dev stack**, prefer `sudo bash scripts/install-dev.sh` (section above) over this binary path.

```bash
# From repo root
go build -o /opt/dockfin-smoke/bin/dockfin ./cmd/dockfin   # or: ./bin/dockfin
cd apps/web && npm run build && cd ../..

# Stop old process
kill "$(cat /opt/dockfin-smoke/api.pid 2>/dev/null)" 2>/dev/null || \
  pkill -f './bin/dockfin serve' || true
sleep 1

# Start (adjust paths to your smoke/workdir)
cd /opt/dockfin-smoke
set -a && source /root/dockfin/.env && set +a
export DOCKFIN_TEMPLATES_DIR=/root/dockfin/templates/compose
export DOCKFIN_WEB_DIR=/root/dockfin/apps/web/dist
nohup ./bin/dockfin serve >>api.log 2>&1 &
echo $! > api.pid
curl -s http://127.0.0.1:8000/api/v1/version
```

**Go-only change (no UI):** rebuild binary + restart serve — skip `npm run build`.  
**UI-only change:** `npm run build` is enough if `DOCKFIN_WEB_DIR` points at `apps/web/dist` (restart API only if it caches aggressively; usually a hard refresh is enough).
...
```bash
cd /data/dockfin
docker compose down -v          # wipes dockfin-pg + dockfin-data
...
```bash
# Stop API first
pkill -f 'dockfin serve' || true

docker compose -f deploy/compose/docker-compose.dev.yml down -v
docker compose -f deploy/compose/docker-compose.dev.yml up -d
# wait until healthy
docker compose -f deploy/compose/docker-compose.dev.yml exec -T postgres pg_isready -U dockfin

set -a && source .env && set +a
go run ./cmd/dockfin migrate   # or: /opt/dockfin-smoke/bin/dockfin migrate
# then start serve again and register a fresh user
```

`down -v` deletes the `dockfin-pg-dev` volume (all projects, users, env vars, etc.).

### Wipe local API data dir

```bash
rm -rf "${DOCKFIN_DATA_DIR:-./data}"/*
# smoke example:
rm -rf /opt/dockfin-smoke/data/*
```

### Clean deployed Docker stacks on the host (smoke VPS)

Containers are named like `dockfin-svc-<id8>-…` with project `dockfin-svc-<id8>`:

```bash
# List
docker ps -a --filter name=dockfin-svc

# Tear down one service project (id prefix = first 8 of service UUID)
docker compose -p dockfin-svc-<id8> -f /data/dockfin/services/<uuid>/docker-compose.yml down -v

# Nuclear: all dockfin service containers + Traefik proxy (careful)
docker ps -aq --filter name=dockfin-svc | xargs -r docker rm -f
docker rm -f dockfin-proxy 2>/dev/null || true
# Optional: remove remote compose dirs
# rm -rf /data/dockfin/services/*
```

Shared Docker network is usually `dockfin`. Do not delete it while Traefik / stacks still need it.
...
```bash
docker exec compose-postgres-1 psql -U dockfin -d dockfin \
...
| Variable | Purpose |
|----------|---------|
| `DOCKFIN_HTTP_ADDR` | Bind address (default `:8000`) |
| `DOCKFIN_DATABASE_URL` | Postgres DSN |
| `DOCKFIN_MASTER_KEY` | ≥32 chars; encrypts secrets |
| `DOCKFIN_PUBLIC_URL` | Panel public URL (cookies / links) |
| `DOCKFIN_PUBLIC_IP` | Magic domain base (`*.IP.sslip.io`) |
| `DOCKFIN_WEB_DIR` | Static SPA directory |
| `DOCKFIN_TEMPLATES_DIR` | One-click templates root |
| `DOCKFIN_DATA_DIR` | Control-plane local data |
| `DOCKFIN_CORS_ORIGINS` | Include Vite origin in dual-process mode |
| `DOCKFIN_COOKIE_SECURE` | `0` for plain HTTP IPs |
| `DOCKFIN_BOOTSTRAP_SELF` | Auto-register this host as a server |
| `DOCKFIN_IMAGE` | Override image for install scripts |
| `DOCKFIN_VERSION` | Tag / build-arg for install scripts |
| `DOCKFIN_DIR` | Install directory override |
...
- Production image source of truth: **GHCR** `ghcr.io/foisalislambd/dockfin` (also mirrored to Docker Hub).
...
cd /data/dockfin && docker compose ps
cd /data/dockfin && docker compose logs -f dockfin
...
tail -f /opt/dockfin-smoke/api.log
